...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        H, W = HW[i]
        K = Ks[i]
        rays_o, rays_d, viewdirs = dvgo_exp.get_rays_of_a_view(
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, jax_key, coarse_ckpt_path=None, writer=None, base_step=0):
    ...
    if reload_ckpt_path is None:
        ...
        model_class = dvgo_exp.DirectVoxGO
    ...
    def gather_training_rays():
        ...
        if cfg_train.ray_sampler == 'in_maskcache':
            ...
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo_exp.get_training_rays(
                rgb_tr=rgb_tr_ori,
                train_poses=poses[i_train],
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
        index_generator = dvgo_exp.batch_indices_generator(len(rgb_tr), cfg_train.N_rand)
    ...
    grid_xyz = torch.stack(torch.meshgrid(
        torch.linspace(model.xyz_min[0], model.xyz_max[0], model.density.shape[2]),
        torch.linspace(model.xyz_min[1], model.xyz_max[1], model.density.shape[3]),
        torch.linspace(model.xyz_min[2], model.xyz_max[2], model.density.shape[4]),
    ), -1)
    with torch.no_grad():
        radius = torch.sqrt((grid_xyz.unsqueeze(0).unsqueeze(1) ** 2).sum(-1))
        radius_mask = radius <= 1.0
        spherical_prior = torch.zeros_like(model.density).to(device)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            ...
            if isinstance(model, dvgo_exp.DirectVoxGO):
        ...
        # Add MSE-based loss
        loss += mse_loss(torch.permute(rgb, (0, 2, 3, 1)), (rgb_tr[sel_b]) / 255.0) * max(1.0, (1 - (global_step / 2000.0)) * 100.0)
        ...
        """
        pred_prob = model.activate_density(model.density)
        ...
        if global_step<cfg_train.tv_before and global_step>cfg_train.tv_after and global_step%cfg_train.tv_every==0:
            if cfg_train.weight_tv_density>0:
                model.density_total_variation_add_grad(cfg_train.weight_tv_density/len(rays_o), global_step<cfg_train.tv_dense_before)
            if cfg_train.weight_tv_k0>0:
                model.k0_total_variation_add_grad(cfg_train.weight_tv_k0/len(rays_o), global_step<cfg_train.tv_dense_before)
